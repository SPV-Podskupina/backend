...
describe('User API Endpoints', () => {
    ...
    describe('GET /user/stats', () => {
        it('should return user statistics', async () => {
            const mockGames = [
                { balance_start: 100, balance_end: 150 },
                { balance_start: 150, balance_end: 140 }
            ];
            ...
            expect(res.body).toHaveProperty('totalEarnings', 40);
    ...
    describe('POST /user/login', () => {
        ...
        it('should log in a user and return token', async () => {
            // Add more explicit mock implementation
            UserModel.authenticate.mockImplementation((username, password, callback) => {
                return callback(null, { _id: 'auth-user-id', username: 'authuser' });
            });

            // Add a console log for debugging in CI
            console.log('UserModel.authenticate mock:', UserModel.authenticate);

            const res = await request(app)
                .post('/user/login')
                .send({ username: 'authuser', password: 'password123' });
            
            // Add response debugging for CI
            if (res.statusCode !== 200) {
                console.log('Login response error:', res.body);
            }
